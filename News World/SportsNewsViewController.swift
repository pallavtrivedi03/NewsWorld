...
class SportsNewsViewController: UIViewController {
    ...
    @IBOutlet weak var sportsNewsTableView: UITableView!
    ...
    override func viewDidLoad()
    {
        ...
        if((UserDefaults.standard.string(forKey: "sports")) != nil)
        {
            selectedSource = (UserDefaults.standard.string(forKey: "sports"))
        }
        else
        {
            selectedSource = ESPN
        }
        sportsNewsTableView.tableFooterView = UIView()
        sportsNewsTableView.register(UINib.init(nibName:"NewsTableViewCell",bundle:nil), forCellReuseIdentifier: "newsTableViewCell")
        callWebServiceForSportsNews()
    }
    
    func callWebServiceForSportsNews()
    {
        newsResponseVO = NewsResponseVO()
        let paramas = ["source":ESPN,"sortBy":top,"apiKey":apiKey]
        ...
        ApplicationController.sharedInstance.webServiceHelper?.getWebServiceWith(url: baseURL, params: paramas, returningVO: newsResponseVO!, completionHandler: { (vo) in
            ...
            DispatchQueue.main.async
                {
                    self.sportsNewsTableView.reloadData()
            }
        })
    }
}
extension SportsNewsViewController:UITableViewDelegate,UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        newsWebViewController?.heading = "ESPN"
